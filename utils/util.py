...
def process_gt(args):
    assert args.divide in ["Seen", "Unseen", "HICO"], "The divide argument should be Seen or Unseen"
...
def normalize_map(atten_map, crop_size):
    ...
    # atten_map = cv2.resize(atten_map, dsize=(crop_size, crop_size), interpolation=cv2.INTER_NEAREST)
